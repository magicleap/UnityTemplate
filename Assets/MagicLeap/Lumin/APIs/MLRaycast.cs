...
// ---------------------------------------------------------------------
// %COPYRIGHT_BEGIN%
// <copyright file="MLRaycast.cs" company="Magic Leap">
//
...
namespace UnityEngine.XR.MagicLeap
{
    ...
    public partial class MLRaycast : MLAPISingleton<MLRaycast>
    {
        ...
        /// </summary>
        private readonly Dictionary<ulong, OnRaycatResultDelegate> pendingQueries = new Dictionary<ulong, OnRaycatResultDelegate>();
        ...
        /// <summary>
        /// Keeps the callbacks that were completed on a specific frame.
        /// </summary>
        ...
        /// \endinternal
        public delegate void OnRaycatResultDelegate(ResultState state, Vector3 hitpoint, Vector3 normal, float confidence);
        #endif
        ...
        /// </returns>
        public static MLResult Raycast(QueryParams query, OnRaycatResultDelegate callback)
        {
            try
            {
                ...
                if (_instance.pendingQueries.Count > 0)
                {
                    foreach (ulong handle in _instance.pendingQueries.Keys)
                    {
                        ...
                        if (resultCode == MLResult.Code.Ok)
                        {
                            ...
                            _instance.pendingQueries[handle](
                                raycastResult.State,
