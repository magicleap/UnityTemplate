...
namespace UnityEngine.XR.MagicLeap
{
    ...
    public partial class MLPersistentCoordinateFrames
    {
        ...
        public partial class PCF
        {
            ...
            /// <summary>
            /// Prevents a default instance of the <see cref="PCF" /> class from being created.
            /// Used for when JSON deserializes MLPCF and we need it's currentResultCode to be Pending.
            /// </summary>
            protected PCF()
            {
            ...
            public void AddBinding(IBinding binding)
            {
                if (binding.PCF == this && !this.bindings.Contains(binding))
                {
                    ...
                    MLPersistentCoordinateFrames.QueueForUpdates(this);
                }
            }
            ...
            public MLResult Update()
            {
                if (MLPersistentCoordinateFrames.IsValidInstance())
                {
                    if (!MLPersistentCoordinateFrames._instance.mapUpdatedPCFsThisFrame.ContainsKey(this.cfuid))
                    {
                        ...
                        MLResult result = this.UpdatePose();

                        if (this.currentResultCode == MLResult.Code.Ok)
                        {
            ...
            /// <summary>
            /// Updates the transform of the PCF based on some recent pose query.
            /// </summary>
            /// <returns>
            /// MLResult.Result will be <c>MLResult.Code.Ok</c> if operation was successful.
            /// MLResult.Result will be <c>MLResult.Code.SnapshotPoseNotFound</c> if the PCF could not be found in the current map.
            /// </returns>
            private MLResult UpdatePose()
            {
                if (MLPersistentCoordinateFrames.IsValidInstance())
                {
                    try
                    {
                        ...
                        {
                            this.CurrentResult = MLResult.Code.Ok;
                        }
                        else
                        {
                            this.CurrentResult = MLResult.Code.SnapshotPoseNotFound;
                        }
                    ...
                    {
                        MLPluginLog.Error("PCF.UpdatePose failed. Reason: API symbols not found.");
                        return MLResult.Create(MLResult.Code.UnspecifiedFailure, "PCF.UpdatePose failed. Reason: API symbols not found.");
                    }
                }
                else
                {
                    MLPluginLog.ErrorFormat("PCF.UpdatePose failed. Reason: No Instance for MLPersistentCoordinateFrames.");
                    return MLResult.Create(MLResult.Code.UnspecifiedFailure, "PCF.UpdatePose failed. Reason: No Instance for MLPersistentCoordinateFrames.");
                }
            }
            ...
            private MLResult UpdateState()
            {
                if (MLPersistentCoordinateFrames.IsValidInstance())
                {
                    try
                    {
                        MLResult.Code resultCode = NativeBindings.MLPersistentCoordinateFramesGetFrameState(MLPersistentCoordinateFrames._instance.nativeTracker, in this.cfuid, out NativeBindings.FrameStateNative nativeState);
            ...
            * Ok & poseChanged       | Ok      | Update
            * Ok & !poseChanged      | Ok      | Stable
            * Ok                     | !Ok     | Lost
            * ----------------------------------------- */
