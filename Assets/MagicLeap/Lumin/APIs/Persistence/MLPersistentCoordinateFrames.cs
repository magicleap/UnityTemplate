...
// ---------------------------------------------------------------------
// %COPYRIGHTBEGIN%
// <copyright file="MLPersistentCoordinateFrames.cs" company="Magic Leap">
//
    ...
    public sealed partial class MLPersistentCoordinateFrames : MLAPISingleton<MLPersistentCoordinateFrames>
    {
        ...
        public static MLResult GetPCFTypeByCFUID(MagicLeapNativeBindings.MLCoordinateFrameUID cfuid, out PCF.Types type)
        {
            ...
            if (MLPersistentCoordinateFrames.IsValidInstance())
            {
                try
                {
                    MLResult.Code resultCode = NativeBindings.MLPersistentCoordinateFramesGetFrameState(_instance.nativeTracker, in cfuid, out NativeBindings.FrameStateNative state);
                    if (!MLResult.IsOK(resultCode))
                    {
                        MLPluginLog.ErrorFormat("MLPersistentCoordinateFrames.GetPCFTypeByCFUID failed to find PCF type. Reason: {0}", MLResult.CodeToString(resultCode));
                        return MLResult.Create(resultCode, string.Format("MLPersistentCoordinateFrames.GetPCFTypeByCFUID failed to find PCF type. Reason: {0}", MLResult.CodeToString(resultCode)));
                    }
                    else
                    {
                        type = state.Type;
        ...
        public static MLResult FindPCFByCFUID(MagicLeapNativeBindings.MLCoordinateFrameUID cfuid, out PCF pcf, bool update = true)
        {
            ...
            if (MLPersistentCoordinateFrames.IsValidInstance())
            {
                ...
                else
                {
                    ...
                    {
                        MLPluginLog.ErrorFormat("MLPersistentCoordinateFrames.FindPCFByCFUID failed to find PCF. Reason: {0}", result);
                    }
        ...
        public static MLResult FindClosestPCF(Vector3 position, out PCF pcf, PCF.Types typesMask = PCF.Types.SingleUserSingleSession | PCF.Types.SingleUserMultiSession | PCF.Types.MultiUserMultiSession, bool update = true)
        {
            ...
            if (MLPersistentCoordinateFrames.IsValidInstance())
            {
                ...
                {
                    MLPluginLog.ErrorFormat("MLPersistentCoordinateFrames.FindClosestPCF failed to get closest PCF. Reason: {0}", result);
                }
                else
                {
                    pcf = pcfList[0];
        ...
        public static MLResult FindAllPCFs(out List<PCF> pcfList, uint maxResults = int.MaxValue, PCF.Types typesMask = PCF.Types.SingleUserSingleSession | PCF.Types.SingleUserMultiSession | PCF.Types.MultiUserMultiSession, bool update = true)
        {
            ...
            if (MLPersistentCoordinateFrames.IsValidInstance())
            {
                ...
                if (!result.IsOk)
                {
                    MLPluginLog.ErrorFormat("MLPersistentCoordinateFrames.FindAllPCFs failed because PCFs could not be found.");
                }
                ...
                try
                {
                    ...
                    if (MLResult.IsOK(resultCode) && numPCFs > 0)
                    {
                        ...
                        else
                        {
                            ...
                            return MLResult.Create(resultCode, string.Format("Failed to get any PCFs. Reason: {0}", MLResult.CodeToString(resultCode)));
                        }
                    }
                    else
                    {
                        MLPluginLog.ErrorFormat("MLPersistentCoordinateFrames.FindPCFsByFilter failed because PCFs could not be found.");
                        return MLResult.Create(resultCode, string.Format("MLPersistentCoordinateFrames.FindPCFsByFilter failed because PCFs could not be found. Reason: {0}", MLResult.CodeToString(resultCode)));
                    }
                }
                catch (EntryPointNotFoundException)
                {
        ...
        internal static IntPtr GetResultString(MLResult.Code result)
        {
            try
            {
                ...
                {
                    MLPluginLog.ErrorFormat("MLPersistentCoordinateFrames.GetResultString failed. Reason: No Instance for MLLocation.");
                }
            }
        ...
        /// Initializes the PCF tracker and registers to head tracking map events.
        /// </summary>
        /// <returns>
        ...
        protected override MLResult StartAPI()
        {
            try
            {
                ...
                if (result.IsOk)
                {
                    MLHeadTracking.RegisterOnHeadTrackingMapEvent(this.HandleMapEvents);
                }
                else
                {
                    MLPluginLog.ErrorFormat("MLPersistentCoordinateFrames.StartAPI failed to start MLHeadTracking. Reason: {0}", result);
                }
        ...
        {
            MLHeadTracking.UnregisterOnHeadTrackingMapEvent(this.HandleMapEvents);
            MLHeadTracking.Stop();
        ...
        /// <summary>
        /// Handles the map event for when a new map session is detected.
        /// This can happen when the device has entered a new map or when the device has initialized a new head pose.
        /// </summary>
        /// <param name="events">The set of map events that could have occurred.</param>
        private void HandleMapEvents(MLHeadTracking.MapEvents events)
        {
            if (IsLocalized && events.IsNewSession())
            {
