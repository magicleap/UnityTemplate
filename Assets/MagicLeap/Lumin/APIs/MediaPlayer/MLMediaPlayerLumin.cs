...
// ---------------------------------------------------------------------
// %COPYRIGHTBEGIN%
// <copyright file="MLMediaplayerLumin.cs" company="Magic Leap">
...
namespace UnityEngine.XR.MagicLeap
{
    ...
    /// <summary>
    /// This class is the video player for the device. It implements the IVideoPlayer interface
    /// </summary>
    internal class MLMediaPlayerLumin : MonoBehaviour, IMediaPlayer
    {
        ...
        public MLResult Play(int localMediaPlayerID)
        {
            ...
            MLResult.Code resultCode = MLMediaPlayerNativeBindings.Play(localMediaPlayerID);
        ...
        public MLResult PauseVideo(int localMediaPlayerID)
        {
            ...
            MLResult.Code resultCode = MLMediaPlayerNativeBindings.PauseVideo(localMediaPlayerID);
            result = MLResult.Create(resultCode);
            if (!result.IsOk)
            {
                MLPluginLog.ErrorFormat("MLMediaPlayerLumin.PauseVideo failed to pause media player {0}. Reason: {1}", localMediaPlayerID, result);
        ...
        public MLResult Seek(int positionMilliseconds, int localMediaPlayerID)
        {
            ...
            MLResult.Code resultCode = MLMediaPlayerNativeBindings.Seek(localMediaPlayerID, positionMilliseconds);
            result = MLResult.Create(resultCode);
            if (!result.IsOk)
            {
                MLPluginLog.ErrorFormat("MLMediaPlayerLumin.Seek failed to seek media player {0}. Reason: {1}", localMediaPlayerID, result);
        ...
        public MLResult SetVolume(float vol, int localMediaPlayerID)
        {
            ...
            MLResult.Code resultCode = MLMediaPlayerNativeBindings.SetVolume(localMediaPlayerID, vol);
            result = MLResult.Create(resultCode);
            if (!result.IsOk)
            {
        ...
        public MLResult Stop(int localMediaPlayerID)
        {
            ...
            MLResult.Code resultCode = MLMediaPlayerNativeBindings.Stop(localMediaPlayerID);
            result = MLResult.Create(resultCode);
            if (!result.IsOk)
            {
                MLPluginLog.ErrorFormat("MLMediaPlayerLumin.Stop failed to stop playing media player {0}. Reason: {1}", localMediaPlayerID, result);
        ...
        public MLResult Resume(int localMediaPlayerID)
        {
            ...
            MLResult.Code resultCode = MLMediaPlayerNativeBindings.Resume(localMediaPlayerID);
            result = MLResult.Create(resultCode);
            if (!result.IsOk)
            {
                MLPluginLog.ErrorFormat("MLMediaPlayerLumin.Resume failed to resume playing media player {0}. Reason: {1}", localMediaPlayerID, result);
        ...
        public MLResult SetLooping(bool loop, int localMediaPlayerID)
        {
            ...
            MLResult.Code resultCode = MLMediaPlayerNativeBindings.SetLooping(localMediaPlayerID, loop);
            result = MLResult.Create(resultCode);
            if (!result.IsOk)
            {
                MLPluginLog.ErrorFormat("MLMediaPlayerLumin.SetLooping failed to set looping for media player {0}. Reason: {1}", localMediaPlayerID, result);
        ...
        public MLResult Cleanup(int localMediaPlayerID)
        {
            ...
            MLResult.Code resultCode = MLMediaPlayerNativeBindings.Cleanup(localMediaPlayerID);
            result = MLResult.Create(resultCode);
            if (!result.IsOk)
            {
                MLPluginLog.ErrorFormat("MLMediaPlayerLumin.Cleanup failed to clean up media player {0}. Reason: {1}", localMediaPlayerID, result);
            ...
            if (!Application.isEditor)
            {
                GL.IssuePluginEvent(MLMediaPlayerNativeBindings.GetRenderCleanupCallback(), localMediaPlayerID);
            }
        ...
        public int GetDurationMilliseconds(int localMediaPlayerID)
        {
            ...
            int duration = 0;
            MLResult.Code resultCode = MLMediaPlayerNativeBindings.GetDurationMilliseconds(localMediaPlayerID, out duration);
        ...
        public int GetPositionMilliseconds(int localMediaPlayerID)
        {
            ...
            int position = 0;
            MLResult.Code resultCode = MLMediaPlayerNativeBindings.GetPositionMilliseconds(localMediaPlayerID, out position);
            if (resultCode != MLResult.Code.Ok)
            {
        ...
        public int GetWidth(int localMediaPlayerID)
        {
            ...
            int width = 0;
            MLResult.Code resultCode = MLMediaPlayerNativeBindings.GetWidth(localMediaPlayerID, out width);
            if (resultCode != MLResult.Code.Ok)
            {
        ...
        public int GetHeight(int localMediaPlayerID)
        {
            ...
            int height = 0;
            MLResult.Code resultCode = MLMediaPlayerNativeBindings.GetHeight(localMediaPlayerID, out height);
        ...
        {
            int bitrate = 0;
            MLResult.Code resultCode = MLMediaPlayerNativeBindings.GetVideoBitrate(localMediaPlayerID, out bitrate);
            MLResult result = MLResult.Create(resultCode);
            if (!result.IsOk)
            {
        ...
        {
            return MLMediaPlayerNativeBindings.GetFrameDropThresholdMs(localMediaPlayerID);
        }
        ...
        {
            MLMediaPlayerNativeBindings.SetFrameDropThresholdMs(localMediaPlayerID, threshold);
        }
        ...
        {
            MLResult.Code resultCode = MLMediaPlayerNativeBindings.SelectSubtitleTrack(localMediaPlayerID, id);
        ...
        {
            MLResult.Code resultCode = MLMediaPlayerNativeBindings.UnselectSubtitleTrack(localMediaPlayerID, id);
            return MLResult.Create(resultCode);
        }
        ...
        public MLResult GetAudioChannelCount(int localMediaPlayerID, out int outAudioChannelCount)
        {
            ...
            MLResult.Code resultCode = MLMediaPlayerNativeBindings.GetAudioChannelCount(localMediaPlayerID, out outAudioChannelCount);
            return MLResult.Create(resultCode);
        }
        ...
        {
            MLResult.Code resultCode = MLMediaPlayerNativeBindings.SetSpatialAudioEnable(localMediaPlayerID, isEnabled);
            return MLResult.Create(resultCode);
        }
        ...
        {
            outIsEnabled = false;
            MLResult.Code resultCode = MLMediaPlayerNativeBindings.GetSpatialAudioEnable(localMediaPlayerID, out outIsEnabled);
            return MLResult.Create(resultCode);
        }
        ...
        {
            MagicLeapNativeBindings.MLVec3f nativePosition = MLConvert.FromUnity(position);
            MLResult.Code resultCode = MLMediaPlayerNativeBindings.SetAudioChannelPosition(localMediaPlayerID, (uint)channel, nativePosition);
            return MLResult.Create(resultCode);
        }
        ...
        {
            MagicLeapNativeBindings.MLVec3f nativePosition = new MagicLeapNativeBindings.MLVec3f();
            MLResult.Code resultCode = MLMediaPlayerNativeBindings.GetAudioChannelPosition(localMediaPlayerID, (uint)channel, out nativePosition);
            outPosition = MLConvert.ToUnity(nativePosition);
            return MLResult.Create(resultCode);
        }
        ...
        public IntPtr GetRenderCallback()
        {
            ...
            return MLMediaPlayerNativeBindings.GetRenderCallback();
        }
        ...
        public MLResult CreateStreamingMediaPlayer(GameObject mediaPlayerGO, string source, int localMediaPlayerID)
        {
            ...
            MLMediaPlayerNativeBindings.MediaPlayerManagedCallbacks managedCallbacks = new MLMediaPlayerNativeBindings.MediaPlayerManagedCallbacks
            {
                OnPrepared = new MLMediaPlayerNativeBindings.PlayerPreparedCallback(PlayerPreparedHandler),
                OnCompletion = new MLMediaPlayerNativeBindings.PlayerCompletionCallback(PlayerCompletionHandler),
                OnBuffering = new MLMediaPlayerNativeBindings.PlayerBufferingCallback(PlayerBufferingHandler),
                OnError = new MLMediaPlayerNativeBindings.PlayerErrorCallback(PlayerErrorHandler),
                OnInfo = new MLMediaPlayerNativeBindings.PlayerInfoCallback(PlayerInfoHandler),
                OnSeekCompleted = new MLMediaPlayerNativeBindings.PlayerSeekCompletedCallback(PlayerSeekCompletedHandler),
                OnVideoSizeChanged = new MLMediaPlayerNativeBindings.PlayerVideoSizeChangedCallback(PlayerVideoSizeChangedHandler),
                OnTrackDRMInfo = new MLMediaPlayerNativeBindings.PlayerTrackDRMInfoCallback(PlayerTrackDRMInfoHandler),
                OnAllTracks = new MLMediaPlayerNativeBindings.PlayerAllTracksCallback(PlayerAllTracksHandler),
                OnSubtitleTracks = new MLMediaPlayerNativeBindings.PlayerSubtitleTracksCallback(PlayerSubtitleTracksHandler),
                OnSubtitleInfo = new MLMediaPlayerNativeBindings.PlayerSubtitle608InfoCallback(PlayerSubtitle608InfoHandler),
                OnSubtitle708Event = new MLMediaPlayerNativeBindings.PlayerSubtitle708EventCallback(PlayerSubtitle708EventHandler),
                OnTimedTextInfo = new MLMediaPlayerNativeBindings.PlayerTimedTextInfoCallback(PlayerTimedTextInfoHandler)
            };

            MLResult.Code resultCode = MLMediaPlayerNativeBindings.InstantiateMediaPlayerContext(localMediaPlayerID, mediaSource, colorSpace, managedCallbacks, this.sharedType, this.sessionID);
        ...
        /// <c>MLResult.Result</c> will be <c>MLResult.Code.MediaGenericNoInit</c> MediaPlayer was not properly built or initialized.
        /// </returns>
        public MLResult SetSharingInfo(int localMediaPlayerID, MLMediaPlayer.SharedType sharedType, string sessionID)
        {
            MLResult result = MLResult.Create(MLResult.Code.Ok);
            this.sharedType = sharedType;
            this.sessionID = sessionID;
            if (this.sharedType == MLMediaPlayer.SharedType.Initiator || this.sharedType == MLMediaPlayer.SharedType.None)
            {
                result = MLResult.Create(MLMediaPlayerNativeBindings.SetSharingInfo(localMediaPlayerID, this.sharedType, this.sessionID));
            }
        ...
        /// </summary>
        /// <param name="localMediaPlayerID">Id of the media player</param>
        [MonoPInvokeCallback(typeof(MLMediaPlayerNativeBindings.PlayerPreparedCallback))]
        ...
        /// </summary>
        /// <param name="localMediaPlayerID">Id of the Media Player</param>
        [MonoPInvokeCallback(typeof(MLMediaPlayerNativeBindings.PlayerCompletionCallback))]
        ...
        /// <param name="localMediaPlayerID">Id of the Media Player</param>
        [MonoPInvokeCallback(typeof(MLMediaPlayerNativeBindings.PlayerBufferingCallback))]
        ...
        /// <param name="localMediaPlayerID">ID of the media player.</param>
        [MonoPInvokeCallback(typeof(MLMediaPlayerNativeBindings.PlayerErrorCallback))]
        ...
        /// <param name="localMediaPlayerID">Id of the media player.</param>
        [MonoPInvokeCallback(typeof(MLMediaPlayerNativeBindings.PlayerInfoCallback))]
        ...
        /// <param name="localMediaPlayerID">Id of the media player.</param>
        [MonoPInvokeCallback(typeof(MLMediaPlayerNativeBindings.PlayerVideoSizeChangedCallback))]
        ...
        /// <param name="localMediaPlayerID">ID of the media player.</param>
        [MonoPInvokeCallback(typeof(MLMediaPlayerNativeBindings.PlayerTrackDRMInfoCallback))]
        ...
        [MonoPInvokeCallback(typeof(MLMediaPlayerNativeBindings.PlayerSeekCompletedCallback))]
        ...
        [MonoPInvokeCallback(typeof(MLMediaPlayerNativeBindings.PlayerSubtitleTracksCallback))]
        ...
        [MonoPInvokeCallback(typeof(MLMediaPlayerNativeBindings.PlayerAllTracksCallback))]
        ...
        [MonoPInvokeCallback(typeof(MLMediaPlayerNativeBindings.PlayerSubtitle608InfoCallback))]
        ...
        [MonoPInvokeCallback(typeof(MLMediaPlayerNativeBindings.PlayerSubtitle708EventCallback))]
        private static void PlayerSubtitle708EventHandler(int mediaPlayerID, ref MLMediaPlayer.Cea708CaptionEventInternal cea708EventInternal)
        {
            ...
            switch (cea708EventInternal.Type)
            {
                case MLMediaPlayer.Cea708CaptionEmitCommand.Buffer:
                    cea708Event.Object = MLConvert.DecodeUTF8(cea708EventInternal.Object, MLCea708CaptionEmitCommandBufferMaxSize);
                    break;
        ...
        [MonoPInvokeCallback(typeof(MLMediaPlayerNativeBindings.PlayerTimedTextInfoCallback))]
        ...
        private IEnumerator RequestActivationKeyRequestCoroutine(byte[] drmUUIDBytes, Action<MLResult, byte[], string> callback)
        {
            ...
            MagicLeapNativeBindings.MLUUIDBytes drm_uuid;
            ...
            {
                MLResult.Code resultCode = MLMediaPlayerNativeBindings.GetActivationKeyRequest(
                    ref drm_uuid,
        ...
        private IEnumerator ProvisionDeviceForDRM(int localMediaPlayerID)
        {
            ...
            {
                this.provisionResult = MLMediaPlayerNativeBindings.GetActivationProvisionRequest(
                    "none",
                ref requestMessagePtr,
                ref requestMessageSize,
            ...
            {
                this.provisionResult = MLMediaPlayerNativeBindings.GetProvisionRequest(
                    localMediaPlayerID,
                    "none",
                    ref requestMessagePtr,
                    ref requestMessageSize,
            ...
            provisionRequest.chunkedTransfer = false;
            ...
            else
            {
                ...
                if (localMediaPlayerID == -1)
                {
                    this.provisionResult = MLMediaPlayerNativeBindings.ProvideActivationProvisionResponse(response, (ulong)response.Length);
                }
                else
                {
                    this.provisionResult = MLMediaPlayerNativeBindings.ProvideProvisionResponse(localMediaPlayerID, response, (ulong)response.Length);
                }
        ...
        private IEnumerator GetKeysAndPrepareDRM(int localMediaPlayerID, MLMediaPlayer.DRMTrack drmTrackEnum)
        {
            ...
            if (this.customLicenseMessageData != null && this.customLicenseMessageData.Count > 0)
            {
                ...
                MLMediaPlayerNativeBindings.SetKeyRequestCustomData(localMediaPlayerID, drmTrackEnum, keys, values, counter);
            }

            this.getKeyResult[drmTrack] = MLMediaPlayerNativeBindings.GetKeyRequest(
                                        localMediaPlayerID,
                                        drmTrackEnum,
                                        MLMediaPlayer.DRMKeyType.Streaming,
                                        ref requestMessagePtr,
            ...
            keyRequest.chunkedTransfer = false;
            //// Defaults to no timeout, best to specify one.
            ...
            else
            {
                ...
                this.getKeyResult[drmTrack] = MLMediaPlayerNativeBindings.ProvideKeyResponse(localMediaPlayerID, drmTrackEnum, response, (ulong)response.Length);
                if (this.getKeyResult[drmTrack] != MLResult.Code.Ok)
                {
                    MLPluginLog.ErrorFormat(
                ...
                this.getKeyResult[drmTrack] = MLMediaPlayerNativeBindings.PrepareDRM(localMediaPlayerID, drmTrackEnum);
                if (this.getKeyResult[drmTrack] != MLResult.Code.Ok)
                {
                    MLPluginLog.ErrorFormat(
        ...
        private IEnumerator PerformDRMHandshakeForDRMTrack(int localMediaPlayerID, MLMediaPlayer.DRMTrack drmTrackEnum)
        {
            ...
            for (int tries = 0; tries < MaxTries; tries++)
            {
                MLResult.Code resultCode = MLMediaPlayerNativeBindings.OpenDRMSession(localMediaPlayerID, drmTrackEnum);
                if (MLResult.Code.MediaDRMNotProvisioned == resultCode)
                {
                ...
                else if (resultCode == MLResult.Code.Ok)
                {
                    ...
                    if (this.getKeyResult[drmTrack] != MLResult.Code.Ok)
                    {
                        ...
                        resultCode = MLMediaPlayerNativeBindings.CloseDRMSession(localMediaPlayerID, drmTrackEnum);
                        if (resultCode != MLResult.Code.Ok)
                        {
                            MLPluginLog.ErrorFormat(
        ...
        private bool CreateTexture(Renderer renderer, int localMediaPlayerID, int width, int height)
        {
            ...
            MLMediaPlayerNativeBindings.SetTextureFromUnity(localMediaPlayerID, this.textureReference.GetNativeTexturePtr(), width, height);
        ...
        private IEnumerator CallPluginAtEndOfFrames()
        {
            while (true)
            {
                ...
                if (!Application.isEditor)
                {
                    ...
                    Native.MLMediaPlayerNativeBindings.GetTrackData(this.mediaPlayerID);
                }
            }
        }
