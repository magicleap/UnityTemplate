...
// ---------------------------------------------------------------------
// %COPYRIGHT_BEGIN%
// <copyright file="MLContacts.cs" company="Magic Leap">
//
...
namespace UnityEngine.XR.MagicLeap
{
    ...
    public sealed partial class MLContacts : MLAPISingleton<MLContacts>
    {
        ...
        public static MLResult AddContact(Contact newContact, out ulong requestHandle)
        {
            try
            {
                if (MLContacts.IsValidInstance())
                {
                    ...
                    MLContactsNativeBindings.ContactNative internalContact = MLContactsNativeBindings.ContactNative.Create();
                    internalContact.Data = newContact;

                    resultCode = MLContactsNativeBindings.MLContactsRequestInsert(ref internalContact, out requestHandle);
        ...
        public static MLResult UpdateContact(Contact updatedContact, out ulong requestHandle)
        {
            try
            {
                if (MLContacts.IsValidInstance())
                {
                    ...
                    MLResult.Code resultCode = MLContactsNativeBindings.MLContactsRequestUpdate(ref internalContact, out requestHandle);
                    result = MLResult.Create(resultCode);
                    if (!result.IsOk)
                    {
                        MLPluginLog.ErrorFormat("MLContacts.UpdateContact failed to update a contact. Reason: {0}", result);
        ...
        public static MLResult DeleteContact(Contact removedContact, out ulong requestHandle)
        {
            try
            {
                if (MLContacts.IsValidInstance())
                {
                    ...
                    MLResult.Code resultCode = MLContactsNativeBindings.MLContactsRequestDelete(ref internalContact, out requestHandle);
                    result = MLResult.Create(resultCode);
                    if (!result.IsOk)
                    {
                        MLPluginLog.ErrorFormat("MLContacts.DeleteContact failed to delete a contact. Reason: {0}", result);
        ...
        {
            MLResult result = MLResult.Create(MLContactsNativeBindings.MLContactsStartup());
            if (!result.IsOk)
            {
        ...
        protected override void CleanupAPI(bool isSafeToAccessManagedObject)
        {
            ...
            try
            {
                if (isSafeToAccessManagedObject)
                {
                    ...
                    {
                        resultCode = MLContactsNativeBindings.MLContactsCancelRequest(_instance.addOperations[i]);
                    ...
                    {
                        resultCode = MLContactsNativeBindings.MLContactsCancelRequest(_instance.updateOperations[i]);
                        if (resultCode != MLResult.Code.ContactsCancelled)
                        {
                    ...
                    {
                        resultCode = MLContactsNativeBindings.MLContactsCancelRequest(_instance.deleteOperations[i]);
                        if (resultCode != MLResult.Code.ContactsCancelled)
                        {
                ...
                resultCode = MLContactsNativeBindings.MLContactsShutdown();
        ...
        private static void ProcessOperations(ref List<ulong> handleList, OperationResultDelegate successAction, OperationResultDelegate failureAction)
        {
            try
            {
                if (MLContacts.IsValidInstance())
                {
                    ...
                    {
                        resultCode = MLContactsNativeBindings.OperationResultNative.GetManagedOperationResult(handleList[listIndex], out OperationResult operationResult);
                        switch (resultCode)
                        {
                            ...
                            case MLResult.Code.InvalidParam:
                                {
                                    ...
                                    resultCode = MLContactsNativeBindings.MLContactsReleaseRequestResources(handleList[listIndex]);
        ...
        private static MLResult.Code GetList(uint pageLength, string pageOffset, out ulong requestHandle)
        {
            try
            {
                if (MLContacts.IsValidInstance())
                {
                    ...
                    MLContactsNativeBindings.ListArgsNative listArgs = new MLContactsNativeBindings.ListArgsNative(pageLength, pageOffset);
                    MLResult.Code resultCode = MLContactsNativeBindings.MLContactsRequestList(ref listArgs, out requestHandle);
                    listArgs.Clean();
                    ...
                    if (!MagicLeapNativeBindings.MLHandleIsValid(requestHandle))
                    {
        ...
        private static MLResult.Code GetSearch(string searchQuery, SearchField searchFields, uint pageLength, string pageOffset, out ulong requestHandle)
        {
            try
            {
                if (MLContacts.IsValidInstance())
                {
                    ...
                    MLContactsNativeBindings.SearchArgsNative searchArgs = new MLContactsNativeBindings.SearchArgsNative(searchQuery, searchFields, pageLength, pageOffset);
                    MLResult.Code resultCode = MLContactsNativeBindings.MLContactsRequestSearch(ref searchArgs, out requestHandle);
                    searchArgs.Clean();
        ...
        private static MLResult.Code GetSelection(SelectionField selectionFields, uint pageLength, out ulong requestHandle)
        {
            try
            {
                if (MLContacts.IsValidInstance())
                {
                    ...
                    MLContactsNativeBindings.SelectionArgsNative selectionArgs = new MLContactsNativeBindings.SelectionArgsNative(selectionFields, pageLength);

                    MLResult.Code resultCode = MLContactsNativeBindings.MLContactsRequestSelection(in selectionArgs, out requestHandle);
                    if (resultCode != MLResult.Code.Ok)
                    {
        ...
        private static MLResult.Code GetListResult(ulong requestHandle, out ListResult listResult)
        {
            try
            {
                if (MLContacts.IsValidInstance())
                {
                    return MLContactsNativeBindings.ListResultNative.GetManagedListResult(requestHandle, out listResult);
                }
                else
                {
        ...
        private static MLResult.Code ReleaseRequest(ulong requestHandle)
        {
            try
            {
                if (MLContacts.IsValidInstance())
                {
                    MLResult.Code resultCode = MLContactsNativeBindings.MLContactsReleaseRequestResources(requestHandle);
                    if (resultCode != MLResult.Code.Ok)
                    {
