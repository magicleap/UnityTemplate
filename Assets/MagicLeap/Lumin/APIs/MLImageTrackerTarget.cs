...
// ---------------------------------------------------------------------
// %COPYRIGHTBEGIN%
// <copyright file="MLImageTrackerTarget.cs" company="Magic Leap">
//
    ...
    public sealed partial class MLImageTracker : MLAPISingleton<MLImageTracker>
    {
        ...
        public sealed class Target : IDisposable
        {
            ...
            public Target(string name, Texture2D image, float longerDimension, OnImageResultDelegate callback, ulong handle, bool isStationary = false)
            {
                ...
                MLResult.Code result = MLImageTrackerNativeBindings.MLImageTrackerAddTargetFromArray(this.trackerHandle, ref this.targetSettings, this.imageData, (uint)image.width, (uint)image.height, (numChannels == 4) ? MLImageTracker.ImageFormat.RGBA : MLImageTracker.ImageFormat.RGB, ref this.targetHandle);
