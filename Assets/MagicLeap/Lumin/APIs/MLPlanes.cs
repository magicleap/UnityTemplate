...
// ---------------------------------------------------------------------
// %COPYRIGHT_BEGIN%
// <copyright file="MLPlanes.cs" company="Magic Leap">
//
        ...
        private void ProcessPendingQueries()
        {
            try
            {
                if (_instance.pendingQueries.Count > 0)
                {
                        ...
                        if (resultCode != MLResult.Code.Pending)
                        {
                            if (resultCode == MLResult.Code.Ok)
                            {
                                ...
                                else
                                {
                                    ...
                                    query.Callback(query.Result, query.Planes, query.PlaneBoundaries);
                                }
                            }
                            else
                            {
                    ...
                    for (int i = 0; i < _instance.completedQueries.Count; ++i)
                    {
                        _instance.pendingQueries[_instance.completedQueries[i]].Dispose();
                        _instance.pendingQueries.Remove(_instance.completedQueries[i]);
                    }
