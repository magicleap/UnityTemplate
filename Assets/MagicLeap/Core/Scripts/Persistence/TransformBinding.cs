...
// ---------------------------------------------------------------------
// %COPYRIGHTBEGIN%
// <copyright file="TransformBinding.cs" company="Magic Leap">
//
...
namespace MagicLeap.Core
{
    ...
    public partial class TransformBinding : MLPersistentCoordinateFrames.PCF.IBinding
    {
        ...
        public bool Update()
        {
            ...
            if (this.PCF == null || this.PCF.CurrentResult != MLResult.Code.Ok)
            {
            ...
            // A = PCF.Orientation
            // B = transform.transform.rotation
            // C = OrientationOffset
            Quaternion relOrientation = Quaternion.Inverse(this.PCF.Rotation) * this.transform.transform.rotation;
            ...
            // B = transform.transform.position
            // C = Offset (Position Offset)
            Matrix4x4 pcfCoordinateSpace = Matrix4x4.TRS(this.PCF.Position, this.PCF.Rotation, Vector3.one);
            Vector3 relPosition = Matrix4x4.Inverse(pcfCoordinateSpace).MultiplyPoint3x4(this.transform.transform.position);
        ...
        public bool Regain()
        {
            ...
            else
            {
                ...
                if (result.IsOk)
                {
                    this.transform.transform.rotation = this.pcf.Rotation * this.offsetOrientation;

                    Matrix4x4 pcfCoordinateSpace = new Matrix4x4();
                    pcfCoordinateSpace.SetTRS(this.pcf.Position, this.pcf.Rotation, Vector3.one);

                    this.transform.transform.position = pcfCoordinateSpace.MultiplyPoint3x4(this.offsetPosition);
                }
                else
                {
                    Debug.LogErrorFormat("Error: TransformBinding failed to regain the binding between PCF and transform. Reason: {0}", result);
